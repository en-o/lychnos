...
风格特点：
- 设计风格：现代信息图海报，布局清晰有序
- 背景：浅色中性色调（米色、浅灰或白色），便于阅读
- 配色方案：和谐的调色板，对比度清晰易读
- 布局：结构化的信息区块，视觉层次清晰
- 字体：简洁现代字体，可读性强；标题加粗，正文常规
- 视觉元素：简洁的插图、图标、图表、流程图辅助内容
- 排版：逻辑化分区，间距和对齐适当
- 装饰元素：最小化装饰，聚焦清晰的信息传达
- 氛围：专业、教育性、信息性、易于理解
- 信息呈现：使用方框、箭头、项目符号和视觉连接线展示关系

**内容提示词使用：**

调用图片生成时，只需提供内容描述（如书名、主题、关键元素），系统会自动应用上述默认风格。例如：
- `generateImageWithContent(userId, contentPrompt)` - 内容描述 + 自动应用默认风格
- `generateImageStreamWithContent(userId, contentPrompt)` - 返回图片流 + 默认风格
