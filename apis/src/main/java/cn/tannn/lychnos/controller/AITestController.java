...
public class AITestController {
    ...
    public ResultVO<ImageResponse> testImage(
        ...
        Long userId = UserUtil.userId2(request);
        log.info("AI图片生成测试，userId: {}, prompt: {}", userId, prompt);

        ImageResponse result = aiService.generateImage(userId, prompt);
        return ResultVO.success(result);
    }
    ...
    public ResultVO<ImageResponse> testImageWithModel(
        ...
        Long userId = UserUtil.userId2(request);
        log.info("AI图片生成测试（指定模型），userId: {}, modelId: {}, prompt: {}", userId, modelId, prompt);

        ImageResponse result = aiService.generateImageWithModel(modelId, userId, prompt);
        return ResultVO.success(result);
    }
}
